...
const buildSealedOrbEffectLines = (originalNames: readonly string[]) => {
  ...
  return functionRecords.flatMap((record) => {
    ...
    const spellLines = spellSummary ?
      [
        `${record.game}에서 문장으로 장착하면 다음 주문을 사용할 수 있습니다. ${spellSummary}`,
      ] :
      [];
