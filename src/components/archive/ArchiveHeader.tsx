...
const ArchiveHeader = ({
  ...
  const activeGroupLabel = useMemo(() => {
    const activeGroup = NAV_GROUPS.find((group) => {
      const groupHrefMatches =
        group.href !== ROUTE_ANCHORS.current && group.href === activeHref;
  ...
  return (
    <header className={CONTAINER_STYLES.header}>
      ...
      {isDrawerOpen ? (
        <div className={HEADER_STYLES.drawerOverlay} onClick={closeDrawer}>
          <aside
            aria-label={ARCHIVE_COPY.header.menuLabel}
          ...
          >
            ...
            <ArchiveSearch variant="drawer" />

            <section className={HEADER_STYLES.drawerSection}>
              <h2 className={HEADER_STYLES.drawerSectionTitle}>
                {ARCHIVE_COPY.header.navigationTitle}
              </h2>
